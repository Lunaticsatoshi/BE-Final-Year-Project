...
def predict_aws_price(input_features):
    new_forecast = {}
    with open("../models/aws_model.pkl","rb") as File:
...
def predict_azure_price(input_features):
    new_forecast = {}
    with open("../models/azure_model.pkl","rb") as File:
        clf = pickle.load(File)
    File.close()
    inputfeatures = input_features
    forecast_list = clf.predict(inputfeatures).tolist()
